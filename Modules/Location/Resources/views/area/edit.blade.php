            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title" style="line-height: 36px;">{{ __('Update State') }}</h3>
                    ...
                    <div class="row pt-3 pb-4">
                        <div class="col-md-6 offset-md-3">
                            <form class="form-horizontal" action="{{ route('module.area.update', $areas->id) }}" method="POST">
                                @csrf
                                    ...
                                    <div class="col-sm-9">
                                        <select name="country_id" id="countryid" class="form-control @error('country_id') is-invalid @enderror">
                                            ...
                                            @foreach ($countries as $country)
                                                <option value="{{ $country->id }}" @if($country->id == $areas->country_id) selected @endif >{{ $country->name }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-9">
                                        <select name="state_id" id="stateid" class="form-control @error('state_id') is-invalid @enderror">
                                            @foreach ($towns as $town)
                                                <option value="{{$town->id}}" @if($town->id == $areas->state_id) selected @endif >{{$town->name}}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="text" name="city_name" placeholder="{{ __('Enter City name') }}" value="{{$areas->city_name}}"  class="form-control @error('city_name') is-invalid @enderror">
