...
Route::prefix('admin/customer')->as('module.customer.')->middleware(['auth:super_admin', 'setlang'])->group(function () {
        ...
        Route::post('/planPurchaseConfirm', [CustomerController::class, 'planPurchaseConfirm'])->name('planPurchaseConfirm');

        Route::get('/status/{customer_id}/{status}', [CustomerController::class, 'status'])->name('status');
});
