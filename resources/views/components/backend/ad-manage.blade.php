    ...
    <tbody>
        @forelse ($ads as $key =>$ad)
            <tr>
                ...
                <td class="text-center" tabindex="0">
                    <button type="button"
                        class="dropdown-toggle btn-sm btn btn-{{ $ad->status == 'active' ? 'success' : ($ad->status == 'pending' ? 'warning' : 'danger') }}"
                    ...
                    <ul class="dropdown-menu" x-placement="bottom-start"
                        ...
                        @if ($ad->status == 'pending' || $ad->status == 'expired' || $ad->status == 'declined')
                            <li><a onclick="return confirm('Are you sure to perform this action?')"
                                    class="dropdown-item" href="{{ route('module.ad.status', [$ad->slug, 'active']) }}">
                        ...
                        @if ($ad->status == 'active')
                            <li><a onclick="return confirm('Are you sure to perform this action?')"
                                    class="dropdown-item"
                                    href="{{ route('module.ad.status', [$ad->slug, 'expired']) }}">
