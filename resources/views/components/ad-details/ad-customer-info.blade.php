<div class="product-item__sidebar-item user-details">
                    ...
                    @if ($customer->image)
                        ...
                        @if ($customer->certified_seller == 1 && $customer->certificate_validity > now())
                            ...
                            <img src="@if ($certified->badge_image) {{ asset($certified->badge_image) }} @else {{ asset('images/certified.jpg') }} @endif"
                                style="width: 25px;height: 25px;border-radius: 50%;position: absolute;bottom: 0px; top: 35px; right: 10px;">
                        @endif
                    @else
    ...
    <ul class="contact">
        ...
        <li class="contact-item">
            ...
            <h6 class="text--body-3" style="text-transform: none">
                @if ($area)
                    {{ $area }} ,
                @endif
                @if ($town)
                    {{ ucwords(strtolower($town->name ?? '')) }}
                @endif
            </h6>
        </li>
