...
<html lang="en">
    ...
    <body
        class="{{ auth('customer')->check() && isset(session('user_plan')->ad_limit) && session('user_plan')->ad_limit < $settings->free_ad_limit? 'wraning-show_hide': '' }}" dir="{{ langDirection() }}">
        <!-- Top bar start  -->
                                ...
                                <ul>
                                    @if(isset($countries) && count($countries) > 0)
                                        @foreach ($countries as $country)
                                            <li>
                                                <a class="nav-link country_name" id="selectCountry" data-id="{{$country->id}}" href="javascript:void(0)">
                                                    {{ Str::ucfirst($country->name) }}
                                                    <i class="fa fa-angle-right"></i>
                                                    @php
                                                        $country_ads_count = DB::table('ads')->where('city_id', $country->id)->count();
                                                    @endphp
        ...
        <!-- Category Modal -->
        <div class="location_modal">
            <div class="modal fade" id="staticBackdropCat" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="staticBackdropLabel">
                            ...
                            <div class="category_list">
                                <ul>
                                    ...
                                    @foreach($categories as $row)
                                        <li>
                                            <a class="nav-link" id="selectCategory" data-id="{{ $row->id }}" href="javascript:void(0)">
                                                 {{ $row->name }}
                                                <i class="fa fa-angle-right"></i>
                                                @php
                                                    $ads_count = DB::table('ads')->where('category_id', $row->id)->count();
                                                @endphp
                                                <span>({{ $ads_count }})</span>
                                            </a>
                                        </li>
